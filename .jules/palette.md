...
## 2025-02-12 - Ensure Proper Cursor on Visually Disabled Elements
**Learning:** In the ESP32 UI (`MJPEG2SD.htm`, `Auxil.htm`, `common.js`), visually disabled elements (`.disabled`) were previously styled using `pointer-events: none` and `cursor: default`. While `pointer-events: none` prevented mouse clicks, it also blocked the CSS hover state, preventing `cursor: not-allowed` from displaying. Additionally, `pointer-events: none` does not prevent keyboard interaction (e.g., Tab focus and Enter key).
**Action:** Remove `pointer-events: none` from CSS for disabled elements to allow hover cursors (`cursor: not-allowed`) to display and provide clear visual feedback. To ensure these elements are properly un-clickable and inaccessible to keyboards natively, explicitly toggle the `el.disabled = true/false` property in Javascript (e.g., in `disableRangeSlider` and `enableRangeSlider`).
