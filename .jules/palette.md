...
## 2024-06-02 - Generalizing ARIA button focus styling
**Learning:** In the ESP32 HTML templates (`MJPEG2SD.htm`, `Auxil.htm`), the `:focus-visible` styles for elements with `role="button"` were specifically scoped to `div[role="button"]`, `nav[role="button"]`, or `button`. Dynamically injected UI components (such as a `span` used as a removal button in `common.js`) with `role="button"` failed to inherit these crucial focus rings, hurting keyboard navigation accessibility.
**Action:** When styling custom UI elements with `role="button"`, use the generic attribute selector `[role="button"]` instead of tag-qualified selectors like `div[role="button"]` to ensure any element acting as a button correctly receives global hover, focus, and pointer cursor states, regardless of its HTML tag.
