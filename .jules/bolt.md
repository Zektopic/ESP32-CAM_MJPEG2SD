...
## 2024-05-31 - Safe O(1) String Formatting in Loops
**Learning:** In C/C++, replacing an O(N^2) `sprintf(buf + strlen(buf), ...)` concatenation with an O(1) tracker like `offset += snprintf(...)` introduces a critical vulnerability if the return value of `snprintf` is not checked. `snprintf` returns the amount of characters that *would* have been written, meaning truncation causes `offset` to exceed the buffer boundaries on the next iteration.
**Action:** When migrating iterative `sprintf` or `snprintf` calls to use an offset tracker in loops, always explicitly validate the returned size dynamically (e.g., `if (written > 0 && written < (int)(sizeof(buf) - offset)) { offset += written; } else { break; }`) to safely prevent out-of-bounds array writes.
