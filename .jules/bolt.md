...
## 2024-06-25 - Optimizing Array Element Offsets and Unrolling in Tight Image Processing Loops
**Learning:** Found a performance issue in `motionDetect.cpp` within `rescaleImage` during bilinear interpolation. The matrix base offset calculations (`yL * inputWidth * colorDepth`) were inside the inner loop and calculated dynamically for every color channel in every pixel.
**Action:** Extract base matrix row additions before the inner loops (`rowL` and `rowH`). Furthermore, branch and unroll the innermost channel loop by detecting common conditions (`colorDepth == RGB888_BYTES` or `GRAYSCALE_BYTES`), performing all calculations natively with flat indexing rather than dynamically iterating.
## 2024-06-25 - Branching and Unrolling Variable-Length Inner Loops
**Learning:** In tight performance-critical loops on the ESP32 (e.g., image processing channels), an inner loop iterating over a runtime-configured but locally constant variable (like `colorDepth`) incurs significant loop overhead and branch prediction misses.
**Action:** Manually unroll the loop into explicit, hardcoded branches (e.g., `if (colorDepth == 3) { ... } else if (colorDepth == 1) { ... }`). By statically mapping the exact sequence of instructions for common constraints, the compiler can optimize away loop structures entirely, providing a noticeable speedup over dynamic iteration.
