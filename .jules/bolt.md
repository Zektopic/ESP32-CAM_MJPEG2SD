...
## 2024-05-31 - Premature optimization in cold paths and snprintf bounds checking
**Learning:** Attempting to eliminate O(N^2) string concatenation in setup functions (like `setupSensors`) using an offset with `snprintf` is a premature optimization because it executes only once and yields zero measurable performance benefit. Furthermore, using `snprintf(buf + offset, MAX_LEN - offset, ...)` is unsafe if the string length can exceed `MAX_LEN`, as `snprintf` returns the number of characters it *would* have written. This can cause the `offset` to exceed `MAX_LEN`, leading to an unsigned integer underflow when calculating `MAX_LEN - offset`, which passes a massive positive number to the next `snprintf` and causes a critical buffer overflow.
**Action:** Avoid micro-optimizations in cold execution paths. When using `snprintf` to avoid redundant `strlen` traversals in hot paths or when simplifying multi-step string operations (e.g., combining `strncpy` and `snprintf`), always clamp the return value before using it in offset calculations (e.g., `if (written >= size) written = size - 1;`) to prevent integer underflows and buffer overflows.
