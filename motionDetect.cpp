  ...
  for (int i = 0; i < outputHeight; ++i) {
    ...
    for (int j = 0; j < outputWidth; ++j) {
      ...
      for (int channel = 0; channel < colorDepth; ++channel) {
        uint8_t a = input[yL_offset + xL_offset + channel];
        uint8_t b = input[yL_offset + xH_offset + channel];
        uint8_t c = input[yH_offset + xL_offset + channel];
        uint8_t d = input[yH_offset + xH_offset + channel];
