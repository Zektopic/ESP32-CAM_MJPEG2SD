    ...
    if (result.classification[0].value > mlProbability) {
      ...
      if (dbgVerbose) {
        ...
        for (uint16_t i = 0; i < EI_CLASSIFIER_LABEL_COUNT; i++)
          // Bolt: Use pointer tracking for O(1) appending, avoiding O(N^2) strlen overhead
          p += sprintf(p, "%s: %.2f, ", ei_classifier_inferencing_categories[i], result.classification[i].value);
