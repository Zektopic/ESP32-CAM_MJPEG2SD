...
static void rescaleImage(const uint8_t* input, int inputWidth, int inputHeight, uint8_t* output, int outputWidth, int outputHeight) {
  // use 16-bit fixed-point arithmetic for bilinear interpolation to optimize performance on ESP32
  uint32_t xRatio = ((uint32_t)inputWidth << 16) / outputWidth;
  uint32_t yRatio = ((uint32_t)inputHeight << 16) / outputHeight;

  int outIdx = 0;
  for (int i = 0; i < outputHeight; ++i) {
    uint32_t srcY = i * yRatio;
    int yL = srcY >> 16;
    int yH = yL + 1;
    if (yH >= inputHeight) yH = inputHeight - 1;
    if ((srcY & 0xFFFF) == 0) yH = yL;

    uint32_t yWeight = (srcY & 0xFFFF) >> 8;
    uint32_t invYWeight = 256 - yWeight;

    int yL_offset = yL * inputWidth * colorDepth;
    int yH_offset = yH * inputWidth * colorDepth;

    for (int j = 0; j < outputWidth; ++j) {
      uint32_t srcX = j * xRatio;
      int xL = srcX >> 16;
      int xH = xL + 1;
      if (xH >= inputWidth) xH = inputWidth - 1;
      if ((srcX & 0xFFFF) == 0) xH = xL;

      uint32_t xWeight = (srcX & 0xFFFF) >> 8;
      uint32_t invXWeight = 256 - xWeight;

      uint32_t wA = invXWeight * invYWeight;
      uint32_t wB = xWeight * invYWeight;
      uint32_t wC = invXWeight * yWeight;
      uint32_t wD = xWeight * yWeight;

      int idxA = yL_offset + xL * colorDepth;
      int idxB = yL_offset + xH * colorDepth;
      int idxC = yH_offset + xL * colorDepth;
      int idxD = yH_offset + xH * colorDepth;

      for (int channel = 0; channel < colorDepth; ++channel) {
        uint32_t a = input[idxA + channel];
        uint32_t b = input[idxB + channel];
        uint32_t c = input[idxC + channel];
        uint32_t d = input[idxD + channel];

        uint32_t pixel = (a * wA + b * wB + c * wC + d * wD) >> 16;
        output[outIdx++] = (uint8_t)pixel;
      }
    }
  }
}
