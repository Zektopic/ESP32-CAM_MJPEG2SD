...
static void onNetEvent(arduino_event_id_t event, arduino_event_info_t info) {
  ...
  switch (event) {
    ...
    case ARDUINO_EVENT_WIFI_STA_GOT_IP: { IPAddress ipSTA = WiFi.STA.localIP(); LOG_INF("Wifi Station IP, use '%s://%u.%u.%u.%u' to connect", useHttps ? "https" : "http", ipSTA[0], ipSTA[1], ipSTA[2], ipSTA[3]); break; }
    ...
    case ARDUINO_EVENT_ETH_GOT_IP: {
      IPAddress ipETH = ETH.localIP();
      LOG_INF("Ethernet IP, use '%s://%u.%u.%u.%u' to connect", useHttps ? "https" : "http", ipETH[0], ipETH[1], ipETH[2], ipETH[3]);
...
static void showBacktrace() {
  ...
  if (haveTrace == MAGIC_NUM) {
    ...
    // Bolt Optimization: Maintain offset to prevent O(N^2) Schlemiel the Painter's algorithm from repeated strlen
    int offset = 0;
    for (int i = 0; i < btLen; i++) {
      int written = snprintf(bt + offset, sizeof(bt) - offset - 11, "0x%08x ", (unsigned int)backtrace[i]); // 11 is size of new trace hex
      if (written > 0 && written < (int)(sizeof(bt) - offset - 11)) {
        offset += written;
      } else {
