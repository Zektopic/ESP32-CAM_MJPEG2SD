...
static void printGpioInfo() {
  ...
  for (uint8_t i = 0; i < SOC_GPIO_PIN_COUNT; i++) {
    ...
    char gpioInf[100];
    char* p = gpioInf;
    int remaining = sizeof(gpioInf);
    int written = 0;
#if defined(BOARD_HAS_PIN_REMAP)
    int dpin = gpioNumberToDigitalPin(i);
    if (dpin < 0) continue;  //pin is not exported
    else {
      written = snprintf(p, remaining, "  D%-3d|%4u : ", dpin, i);
      if (written > 0 && written < remaining) { p += written; remaining -= written; }
    }
#else
    written = snprintf(p, remaining, "  %4u : ", i);
    if (written > 0 && written < remaining) { p += written; remaining -= written; }
#endif
    const char *extra_type = perimanGetPinBusExtraType(i);
    if (extra_type) written = snprintf(p, remaining, "%s", extra_type);
    else written = snprintf(p, remaining, "%s", perimanGetTypeName(type));
    if (written > 0 && written < remaining) { p += written; remaining -= written; }

    int8_t bus_number = perimanGetPinBusNum(i);
    if (bus_number != -1) {
      written = snprintf(p, remaining, "[%u]", bus_number);
      if (written > 0 && written < remaining) { p += written; remaining -= written; }
    }

    int8_t bus_channel = perimanGetPinBusChannel(i);
    if (bus_channel != -1) {
      written = snprintf(p, remaining, "[%u]", bus_channel);
      if (written > 0 && written < remaining) { p += written; remaining -= written; }
    }
    *p = 0;
    LOG_SEND("%s\n", gpioInf);
